        ...
        Vault.recurseChildren(plugin.app.vault.getRoot(), async (file: TAbstractFile) => {
            if (file instanceof TFolder && file.isRoot()) {
                ...
                if (file instanceof TFolder) {
                    await fs.mkdir(path.posix.join(plugin.settings.exportPath, file.path));
                } else {
                    await fs.copyFile(path.posix.join(vaultAbsolutePath, file.path), path.posix.join(plugin.settings.exportPath, file.path));
                }
            }
        })
